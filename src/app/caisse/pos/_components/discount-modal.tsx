...
export function DiscountModal({ ticket, onApply, onClose }: DiscountModalProps) {
  ...
  // Le serveur valide les remises fixes contre subtotalHt (pas TTC) — même base ici
  ...
  const handleApply = () => {
    ...
    if (!v || v <= 0) return
    if (type === 'amount') {
      // Plafonner à subtotalHt - 0.01 (server rejette value > subtotalHt, paiement rejette total <= 0)
      const capped = Math.min(v, Math.max(0, subtotalHt - 0.01))
      if (capped <= 0) return
